...
if __name__ == '__main__':
    root = "/mount/projekte/textklang/Primary-Data/Schallplattendigitalisate-for-pipeline"

    for f in tqdm(os.listdir(root)):
        if f.endswith(".wav"):
            audio_path = os.path.join(root,f)
            poem_id = f.split(".")[0]
            text_path = f"{root}/{poem_id}-text.txt"
            print(poem_id)
            print(audio_path)
            print(text_path)
            out_dir = f"/mount/arbeitsdaten/textklang/synthesis/styles/{poem_id}"
            if not os.path.exists(out_dir):
                split_audio(audio_path, text_path, out_dir, max_num_lines=100, n=3)
