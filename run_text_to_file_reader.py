...
def read_texts(model_id, sentence, filename, device="cpu", language="en", path_to_reference_audio=None):
    tts = InferenceFastSpeech2(device=device, model_name=model_id)
    tts.set_language(language)
    if path_to_reference_audio is not None:
        tts.set_utterance_embedding(path_to_reference_audio)
...
if __name__ == '__main__':
    ...
    read_texts(model_id="Wunderhorn", sentence="Ein Knab auf schnellem Roß, Sprengt in der Kaisrin Schloß", filename="audios/test_wh.wav", device="cpu", language="de")
